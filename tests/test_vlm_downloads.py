...
class VlmDownloadPreflightTests(unittest.TestCase):
    def test_qwen_2b_is_registered_for_preflight(self):
        self.assertEqual(VLM_MODELS["Qwen3-VL-2B-Instruct-4bit"], "qwen_2b")
        self.assertEqual(MODEL_SIZE_GB["Qwen3-VL-2B-Instruct-4bit"], 1.78)

    def test_titles_always_match_checkpoint_basenames(self):
        # dropdown text == CLI preflight title == HF cache dir basename; labels
        # are derived from backend_model_id so they can never drift apart
        from pave_mlx.backends import backend_model_id
        from pave_ui import perception

        for models in (VLM_MODELS, perception.VLM_MODELS):
            for label, key in models.items():
                self.assertEqual(label, backend_model_id(key).rstrip("/").split("/")[-1])

    def test_ab_candidates_are_registered_for_preflight(self):
        self.assertEqual(VLM_MODELS["Qwen3.5-2B"], "qwen35_2b")
        self.assertEqual(MODEL_SIZE_GB["Qwen3.5-2B"], 4.5)
        self.assertEqual(VLM_MODELS["Fourier-Qwen2-VL-2B-0.67"], "fourier_qwen2vl_2b")
        self.assertEqual(MODEL_SIZE_GB["Fourier-Qwen2-VL-2B-0.67"], 4.42)
    ...
    def test_gemma_report_uses_indexed_weight_total_and_config_shape(self):
        with tempfile.TemporaryDirectory() as tempdir, patch.dict(os.environ, {"HF_HOME": tempdir}):
            ...
            report = model_download_report("gemma-4-E4B-it-MLX-4bit")
    ...
    def test_gemma_preflight_prints_shared_kv_compat_note(self):
        with tempfile.TemporaryDirectory() as tempdir, patch.dict(os.environ, {"HF_HOME": tempdir}):
            ...
            with contextlib.redirect_stdout(stream):
                print_model_download_report("gemma-4-E4B-it-MLX-4bit", stream=stream)
